 ...
 * ```
 **/
export type BICAdapter = Adapter & {
  getAccountByProviderAndUserId({ provider, userId }: {provider: string, userId: string}): Awaitable<AdapterAccount | null>
}
...
export function DynamoDBAdapter(
  ...
  return {
    ...
    async getAccountByProviderAndUserId({ provider, userId }: {provider: string, userId: string}) {
      ...
      const account = data.Items[0] as AdapterAccount
